...
# Function to read the config file and load configuration
...
def get_prompt(resume, label, tokenizer, max_length=2000):
...
class CustomDataset:
    def __init__(self, df, tokenizer, batch_size=1, max_length=2000, balanced_sampling=True):
        self.df = df
        ...
        while len(data) <= self.batch_size:
            if self.balanced_sampling:
                ...
                if self.batch_size == 1:
                    ...
                    label = self.df.iloc[i]["label"]
                    resume = self.df_0.iloc[i]["resume"][:6500]
                    prompt = get_prompt(resume, label, self.tokenizer, self.max_length)
                    if prompt:
                        data.append(prompt)
...
class CustomTrainer:
    ...
    def find_accuracy(self, logits, targets, verbose=False):
    ...
    def run_test_batch(self):
        ...
        with torch.no_grad():
            ...
            overall_precision = precision_score(y_true_accumulated, y_pred_accumulated, pos_label=1, zero_division=1)
            overall_recall = recall_score(y_true_accumulated, y_pred_accumulated, pos_label=1, zero_division=1)
            overall_f1 = f1_score(y_true_accumulated, y_pred_accumulated,  pos_label=1, zero_division=1)
    ...
    def train(self):
        for epoch in range(self.num_epochs):
            ...
            for batch in range(len(self.train_dataset) // self.batch_size):
                curr_batch_no += 1
                input_tokens, targets = prepare_batch(
                ...
                loss.backward()
                if curr_batch_no % self.optimize_step == 0:
                    self.optimizer.step()
                    self.optimizer.zero_grad()
...
@click.command()
@click.option("--config", default="src/configs/classifier.toml", help="Path to the config TOML file")
def main(config):
    config = load_config(config)

    model, tokenizer = load_model(config["model"]["model_id"])
    max_length = config["model"]["max_length"]
    batch_size = config["model"]["batch_size"]
    optimize_step = config["model"]["optimize_step"]

    train, test = get_dataset(config["dataset"]["dataset_json"])
    train_dataset = CustomDataset(train, tokenizer, batch_size, max_length=max_length)
    test_dataset = CustomDataset(test, tokenizer, batch_size, max_length=max_length, balanced_sampling=False)
